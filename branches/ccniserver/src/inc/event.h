...
public:
    ...
    bool wait(pthread_mutex_t & mu, int millisecond = -1)
    {
        struct timeval now;
        ...
        if (millisecond > 0)
        {
            ...
            retcode = pthread_cond_timedwait(&_cond, &mu, &timeout);
        }
        else
        {
            retcode = pthread_cond_wait(&_cond, &mu);
        }
    ...
    void signal(pthread_mutex_t & mu)
    {
        //pthread_mutex_lock(&mu);
